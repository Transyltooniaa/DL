...
def main(args):
    ...
    # Try to load fallback prompt tensor if present (prompt_embedding.pt)
    # This supports the ES path which may save only a prompt tensor instead of a PEFT checkpoint.
    possible_prompt_paths = [
        os.path.join(args.model_checkpoints, 'prompt_embedding.pt'),
        os.path.join(path, 'prompt_embedding.pt'),
    ]
    loaded_prompt = False
    for ppath in possible_prompt_paths:
        if os.path.exists(ppath):
            try:
                prompt_tensor = torch.load(ppath, map_location='cpu')
                # find matching parameter in model
                matched = False
                for n, param in model.named_parameters():
                    lname = n.lower()
                    if ('prompt' in lname or 'virtual' in lname or 'prompt_encoder' in lname):
                        if tuple(param.shape) == tuple(prompt_tensor.shape):
                            param.data.copy_(prompt_tensor.to(param.device))
                            matched = True
                            break
                # fallback: match by first dim == ptuning_num_tokens
                if not matched:
                    for n, param in model.named_parameters():
                        try:
                            if param.shape[0] == int(params.get('ptuning_num_tokens', 0)) and param.dim() == prompt_tensor.dim():
                                param.data.copy_(prompt_tensor.to(param.device))
                                matched = True
                                break
                        except Exception:
                ...
                if matched:
                    print(f"Loaded prompt tensor from {ppath} into model parameter {n}")
                else:
                    print(f"Found prompt tensor at {ppath} but could not match it to any model parameter. Saved shape={prompt_tensor.shape}")
                loaded_prompt = True
                break
            except Exception as e:
                print(f"Failed to load prompt tensor from {ppath}: {e}")
                continue
